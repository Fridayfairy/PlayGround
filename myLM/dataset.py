...
class PretrainDataset(Dataset):
    ...
    Args:
        data_path (str): 数据集路径
        tokenizer (PreTrainedTokenizerFast): 分词器
        max_length (int, optional): 最大序列长度. Defaults to 512.
    将text通过tokenizer转成input_id,然后将input_id 拆分成X Y
    X 为 input_id[:-1] 个token, Y 为 input_id[1:] 个token
    X 模型输入（除去最后一个位置）
    Y 模型标签（右移后除去第一个位置）
    '''
    ...
    def __getitem__(self, index: int):
        ...
        # 1 表示有效位置，0 表示 Padding，对应 Y 的每个位置。
